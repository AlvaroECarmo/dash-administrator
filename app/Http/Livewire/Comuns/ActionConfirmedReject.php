...
class ActionConfirmedReject extends ActionConfirmed
{
    ...
    public function rejected()
    {
        ...
        $justificacao->updateObservacoes($this->observacoes, '[ Rejeitado ] - ');
        $justificacao->update(['estado' => 2]);
        WorkList::where('justificacaofalta_id', $this->justificacao['id'])->delete();
        $this->reset();
        $this->init();

        $this->dispatchBrowserEvent('close-formManager', ['message' => config('Departments.ActionRejected')]);
    }
